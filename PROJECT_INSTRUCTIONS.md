...
15. 写作治理以两个 Authority 收口：`core/writing_reasoning_contract.yaml` 管理 Formula Trace、Algorithm Trace 与 `not_needed / stepwise / pseudocode`、Hard / Default / Recommendation、命题预算、Terminology、Numeric Style、Title Claim、深化证据处置、Paragraph Necessity、Paper Fragment stale、Citation Evidence，以及模型建立—求解—结果解释的连续数学叙事；`modules/05_writing/latex.md` 管理正文结构与表达。模型建立与求解默认避免重复问题分析/模型假设，核心公式沿“当前需要—建式依据—关系—结构后果—下一用途”自然推进，solver 从真实模型结构/计算困难中引出，关键结果在邻近位置解释；标题按独立数学任务组织而不按合同字段机械拆分。**既定论文大章节骨架保持不变，各问题章节内部再按真实局部数学任务与求解依赖组织二级/三级小节，并按决定性分配详略、就近闭合图结果证据；局部顺序不得升级为重排问题章节的权限。** `packs/artifact/algorithm_flow.md` 和命题 Pack 只提供按需呈现细则，DOCX、AI cleanup、review、Artifact Packs 和检查表只消费 Authority，不得重新定义第二套正文规则；
