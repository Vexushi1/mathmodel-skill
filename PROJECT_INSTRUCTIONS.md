...
15. 写作治理按 v8 单一职责收口：CUMCM 一级结构与模板槽位由 `templates/latex/cumcm/hsk/template_manifest.yaml` 管理；普通正文组织由 `modules/05_writing/paper_writing_protocol.md` 管理；Formula Trace、Algorithm Trace、Model/Solver/Validator、Claim Strength、Numeric、Terminology、Citation 与 stale 等复杂语义由 `core/writing_reasoning_contract.yaml` 管理；`modules/05_writing/latex.md` 只负责 LaTeX 环境、文件、引用和审计/编译接口。模型建立与求解默认避免重复问题分析/模型假设，核心公式沿“当前需要—建式依据—关系—结构后果—下一用途”自然推进，solver 从真实模型结构/计算困难中引出，关键结果在邻近位置解释；标题按独立数学任务组织而不按合同字段机械拆分。Template Manifest 确定的**既定论文大章节骨架保持不变**；各问题章节内部按真实局部数学任务与求解依赖组织二级/三级小节，并按决定性分配详略、就近闭合图结果证据；局部顺序不得升级为重排一级骨架的权限。`packs/artifact/algorithm_flow.md` 和命题 Pack 只提供按需呈现细则，DOCX、AI cleanup、review、Artifact Packs 和检查表只消费 Authority，不得重新定义第二套规则；
