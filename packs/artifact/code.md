...
基础布局是主求解 Python、`问题X求解结果.xlsx` 与 `qX_plot.m`；Gate=`required` 时再追加结果深化分析 Python 与 `问题X结果深化分析.xlsx`。因此只有 Gate=`required` 路径才具有两个阶段明确的 Python 文件；Gate=`not_required` 时仅保留主求解 Python。Gate=`required` 的兼容五文件路径中会形成同目录两个真实工作簿（主工作簿 + 03B 工作簿）；该表述仅描述被 Gate 激活的路径，不适用于 Gate=`not_required`。主工作簿 accepted 后冻结主求解脚本，不用深化分析代码覆盖它。Gate=`not_required` 时记录非空理由，不生成 03B 代码/工作簿，也不得把缺少 03B 解释为稳健性或稳定性已验证。
