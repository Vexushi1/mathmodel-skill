...
- 数据驱动结果 Figure 的 palette profile、open-axis、legend 与 adaptive canvas 只服从 Module 04 的 Publication Rendering Grammar：少量核心对象默认 `competition_high_contrast`，密集多方法/多 panel 可用 `journal_balanced`，黑白/色觉安全场景可用 `monochrome_print`；辅助对象、CI、背景和参考元素始终降权；
