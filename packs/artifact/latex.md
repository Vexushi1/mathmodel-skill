...
用户要求 LaTeX、可编译终稿、PDF 终稿或从 DOCX 迁移时加载。进入终稿前，项目根目录 `模型论文框架.md` 必须为 current；模型口径、核心结果、约束检验、主要图表和参考文献应已锁定。
...
- 当前模型、逐问结果摘要、图表映射和论文结构必须能追溯到 `模型论文框架.md`。
...
- 不得恢复 `模型论文框架.md` 已删除的旧模型、旧参数、旧约束或旧结果。

## 验收条件

- `.tex`、模板类文件、图片、`.bib`、PDF 和完整最新版 `模型论文框架.md` 均交付，不只交 PDF；
- 框架通过 `scripts/validate_model_paper_framework.py`，已求解小问结果摘要均为 current；
