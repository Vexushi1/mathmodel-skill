...
结构标签只有在它改变变量、约束、验证方法或交付物时才保留。空间、网络、调度和博弈不再与“预测/优化/评价”争夺主题型位置。
...
Capability 决定必须输出的工作表和验证证据，不能仅凭题型名称机械推断。
...
问题一:
  classification:
    ...
    legacy_task_packs: [prediction, spatial]
  reason:
...
旧版 `problem_types.primary/secondary` 可由 `legacy_task_packs` 兼容生成，但新项目以 `classification` 为准。

高级方法不是题型标签；只有明确提出高级方法或路线B作为主模型时才加载 `advanced_method_gate.md`。
