...
A0裁决先行提交为 `b169b0e5046d6fde0748fc7b752641da2f0dedef`，实施PR为 #237，基线为原计划合并提交 `870e3345f1d3180b80be7a8fdf31f41ae00bee85`（文件树9255d6e32afbf40034c102ee06f296cd067fcfd4）。A1已完成下述本地实现和完整回归；本节为提交验收快照，远程最终head、是否合并及merge SHA以PR #237最后验收记录为准，不以早期候选状态代替后续事实。本轮只发布稳定v10.1.0，不自动发布10.2.0。

实际记录：v10.1.0基线1730项/3跳过无失败；A1首轮26专项有1失败/1错误，分别为null新声明被误作缺失、测试表达式被既有SIB占位符规则拒绝。修正null逻辑并改用不含占位符的危险表达式fixture后26项通过。第一轮完整1756项/306.910秒/3跳过，有9失败，分别为当前版本断言、已明确变更的Schema保护摘要和bootstrap超出7000字节预算；未提高预算，而是移除重复入口导航。旧Schema去除A1新增定义/字段后与基线的规范化JSON SHA-256完全一致，单独测试保护这一点。

A1随后补充策略文件并发变化、嵌套MATLAB未知边界、语义变更、non-executable处置、旧流程不加载新模块，以及精确10.2版本测量转换的正反例。第二轮完整1765项/314.519秒/3跳过无失败。随后将配置解析绑定到同一次捕获的源码字节，避免额外读取产生时点混合，并新增Python/MATLAB对应反例；最终36项专项通过，完整1766项/312.948秒/3跳过无失败，lint与生成索引检查均exit0。19个既有P2场景保留原始差异，只登记精确10.2版本变更后没有未说明的字段差异；不能把该结果解释为所有未来场景无误。

只读小型合成测量每后端5次，2个源文件和8个模型选择器：Python中位0.105514秒，MATLAB词法检查中位0.106059秒；这不是原生MATLAB运行证据或一般性能承诺。最终远程精确head的全13项CI与优化基线仍须在本PR最后记录核对；不能用本地结果或此前绿色代替。

原计划52场景中A1只覆盖静态/快照子范围；运行验收及跨模块场景继续留给A2及后续阶段。
