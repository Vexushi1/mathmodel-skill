...
>
> **当前进度：S0—S4 的实现和本地验收已完成，S5 最终远程 CI 核对以 PR #236 的精确最终 head 为准。** 本地最终完整测试 1724 项，3 项条件跳过，无失败；这不替代原生 MATLAB、Windows 或其他 Python 版本的远程结果。不创建 Release，不移动 tag，不修改任何真实用户项目。
...
| S2 | AUD-01/AUD-03 资格漏检 | 实际输入与严格类型，未制造批准/验收 | 本地完成，纳入完整1724测试 |
| S3 | AUD-02 辅助输入身份闭环 | 合同、解析、实例、receipt、sync、package、迁移 | 本地完成；Python 原生已跑，MATLAB 原生由正式CI追加验证 |
| S4 | AUD-04/AUD-05 入口和职责 | 新中立、旧兼容、委托唯一 | 本地完成，差分与文档回归通过 |
| S5 | 版本/生成物/最终远程验证 | lint、完整测试、精确最终head CI、无临时流程 | 本地完成；远程最终判定见 PR #236 验证记录 |
...
| 最终本地完整回归 | 1724项，3跳过，无失败，309.251s | 加入直接writer非法辅助协议拒绝后再次全量 |
| 最终本地lint/索引 | 均exit0 | 生成文件由生成器产生，未手改hash |
...
| 最终远程CI | 读取PR精确最终head的真实记录 | 本地通过不替代远程，多次中间run不作最终证明 |
...
- [x] AUD-04新中立/旧兼容差分通过。
- [x] AUD-05职责委托回归通过。
- [x] 1724项本地完整测试及lint/索引通过。
- [x] 两份临时workflow从分支删除，正式CI保留新增原生测试。
- [ ] 精确最终head的全部远程CI与MATLAB原生结果已核实；以PR最后验收记录为准。
- [ ] 完成最终差异审阅和合并状态登记；未合并时不得声称main已更新。
