...
1. 问题重述默认采用“问题背景 + 问题提出”；问题背景通常一个自然段，问题提出按“问题一：”“问题二：”逐问用自己的理解转述研究对象、关键条件和待求输出；
2. 问题分析按问分小节，只写难点、对象关系、跨问依赖和建模抓手，不写正式公式与最终结果；
3. 模型假设与符号说明分章；各问使用“问题X模型建立及求解”；详细推导后、求解前设置“核心模型汇总”；
4. 每问主结果默认放在“求解结果”小节，深化证据按真实方法命名；不机械设置“小问结论”，最后一个结果/深化段自然回答设问；
5. 中文国赛默认不设置全文独立“结论”一级章；只有当届模板、用户或论文类型明确要求时增加；
6. 命题短证明“分段优先、分点按需”：连续推理用自然段和必要公式，多阶段证明才使用 2--6 个编号步骤；命题显示编号使用“命题 4.1、命题 6.2”等阿拉伯章节号，命题框原则上不跨页；机器契约只要求逻辑单元清晰，不把“清晰”解释成所有证明必须编号分步；
7. 三线表内容默认居中，表题在上、图题在下；正文核心图、核心表必须有邻近的显式编号引用和解释；
8. 模型评价采用“模型的评价与推广”或确有改进时采用“模型的改进、评价与推广”；
9. AI 清理后执行略朴素、生涩但规范的科研初学者式学术重写，并以正向叙述为主；高密度“但/然而/不是/不能/只能”等结构必须复查真实冲突，不把科研初学者式写成频繁自我否定和免责声明；
10. 成稿进入编译前运行 `scripts/audit_paper_prose.py`。默认报告 warning/review_required；warning 只提示人工复查，`--strict` 只阻断独立结论章、H1/A1、缺核心模型汇总等结构性 review_required，不自动改写正文。
