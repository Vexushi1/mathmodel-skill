...
默认顺序：审题 → 模型设计与锁定 → 创建或重写 `模型论文框架.md` → Python 求解验证 → 两类中文结果工作簿 → 多算法验证、敏感性与鲁棒性检验 → 同步逐问结果摘要 → MATLAB 结果图与核心机理图精修 → 直接编写并持续修改 LaTeX 草稿 → AI 模板感清除 → 编译 → 终审提交。

每问至少闭合：目标与依赖、两条实质路线、数据协议、变量、3--5 个关键假设、核心公式与约束、Python 实现、适用检查、敏感性与鲁棒性、结果工作簿、图表证据和最终结论。

任务可直接跳转，但不得伪造缺失前置结果。已有可靠成果必须复用。DOCX 仅在用户明确要求 Word 审阅、批注、协作或特定提交格式时作为独立分支加载。
...
- LaTeX：从首个论文正文版本开始直接编写并持续修改，承担终稿与 PDF。
- DOCX：仅在用户明确要求 Word 审阅、批注或特定提交格式时作为独立可选分支，不得成为默认流程或 LaTeX 前置。
...
LaTeX 必须直接读取当前框架、标准工作簿和已批准图表，从首个论文正文版本开始在源码中持续修改。LaTeX 草稿必须先清除空泛价值判断、机械连接词、无证据的“显著提高”、无作用命题和模板化检查表，再进行最终编译。DOCX 仅在显式请求时独立生成，不提供 LaTeX 前置事实源。中文国赛保留 `cumcmthesis`。
