...
- `lint_skill.py`：检查活动版本、路由、语义治理、通用判定式条件数据预处理、产物闭环、五文件合同、代码质量合同、写作策略合同、Schema、兼容层隔离、仓库引用路径、本地 Markdown 链接、全路由 resolver smoke、旧结构残留、Python 语法和生成文件；
...
v7.4.3 的正文写作策略由 `modules/05_writing/latex.md` 唯一负责，`modules/05_writing/ai_cleanup.md` 执行去模板化检查，模板与回归测试只实现/验证该权威规则。重点包括逐问问题分析、假设/符号分章、核心模型汇总、分段且不跨页的命题短证明、表上图下以及科研初学者式学术重写。

v7.4.2 引入的 Figure Evidence 动态布局和高对比配色继续保持：绘图代码生成前动态判断单图、1×2、2×1、1×3、2×2或拆图，不在脚本层写死某一种布局；主比较允许中高饱和、高对比配色，辅助元素降权。
