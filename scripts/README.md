...
- `lint_skill.py`：检查活动版本、路由、语义治理、通用判定式条件数据预处理、`data_process.m`预处理图证据、产物闭环、五文件合同、代码质量合同、Schema、旧结构残留、Python 语法和生成文件；
...
- `sync_project.py`：按 active data source 发现条件式预处理产物与每问两个阶段脚本、两个结果工作簿和 `qX_plot.m`；figures scope 下 `project_level` 额外检查 `数据预处理/data_process.m`，隔离各层哈希并传播产物 stale；
...
若为 `project_level`，Python 预处理工作簿还必须保存处理前后、修复验证、采样覆盖或结构对齐等真实底层证据；后续 MATLAB Figure Evidence 阶段生成固定名称 `data_process.m`，只读统一工作簿绘图，不重新执行任何预处理。
