...
- `lint_skill.py`：检查活动版本、路由、产物闭环、四文件合同、代码质量合同、Schema、旧结构残留、Python 语法和生成文件；
- `resolve_workflow.py`：解析意图、`objective`、`structures`、`capabilities` 与竞赛类型，返回确定性执行计划；
- `validate_code_delivery.py`：静态校验每问唯一 Python 脚本的完整运行配置与工程质量，不运行赛题代码；
- `validate_user_execution.py`：验收用户返回的两个工作簿及运行配置、哈希和数值质量门；
...
每问 Python 主链为：主求解版本 → 代码工程质量门 → 用户运行 → 主工作簿验收 → 覆盖更新同一脚本 → 分析阶段 → 再次代码质量门 → 深化工作簿验收。运行配置嵌入 Python 和工作簿，不产生独立配置、说明或校验报告。
