...
v7.4.4 的正文写作仍由 `modules/05_writing/latex.md` 唯一负责，`modules/05_writing/ai_cleanup.md` 只执行模板化和自然度检查。当前重点包括：问题重述使用“问题背景 + 问题提出”，问题提出逐问转述；正文以正向连续叙述为主并复查否定/转折密度；每问保留“核心模型汇总”和“求解结果”；核心图表必须有邻近显式编号引用和解释；命题证明采用“分段优先、分点按需”；中文国赛默认不设置固定“小问结论”和全文独立“结论”章。

v7.4.3 建立的假设/符号分章、自然假设编号、短上标优先、表上图下、三线表短内容居中和模型评价规范继续保持；v7.4.2 引入的 Figure Evidence 动态布局和高对比配色也继续保持。绘图代码生成前动态判断单图、1×2、2×1、1×3、2×2或拆图，不在脚本层写死某一种布局；主比较允许中高饱和、高对比配色，辅助元素降权。
