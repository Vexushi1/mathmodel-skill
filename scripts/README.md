...
v7.4.5 保留 v7.4.4 的自然论文流，并清理了证明机器契约的旧歧义：默认 `paragraph_first`，要求逻辑单元清晰，只有明显多阶段证明才使用 2--6 个编号步骤。新增 prose audit 检查高密度否定/转折、重复段首主语、重复固定图表句式及独立结论章、H1/A1、缺“问题提出”/“核心模型汇总”等结构回退；普通单次使用“但/然而”不判错。
