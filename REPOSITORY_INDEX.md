# Mathmodel Skill Repository Index v6.2.5
...
| `core/output_contract.yaml` | 项目目录、`模型论文框架.md`、文件名、MATLAB 标题、软件职责与元数据条件 |
| `core/workbook_schema.yaml` | 工作表、字段、单位、非空、capability 和 MATLAB 交接 |
| `core/project_state.schema.yaml` | 每问状态、题型、能力、框架/结果摘要状态、哈希、证据与阶段门槛 |
...
| 模型论文框架、结果摘要或模型变更同步 | `modules/02_model_design.md` | `templates/model/model_paper_framework.md` |
| 高级方法 | `modules/02_model_design.md` | `packs/task/advanced_method_gate.md` + 对应题型 Pack |
| Python 求解、优化、仿真、统计检验 | `modules/03_solve_validate.md` | `packs/artifact/code.md`、工作簿 Schema、结果摘要同步 |
...
| Word/DOCX 草稿 | `modules/05_writing/docx.md` | `templates/writing/docx_check.md`、当前框架 |
| LaTeX 终稿与编译 | `modules/05_writing/latex.md` → `ai_cleanup.md` → `modules/05_latex_compile_quality.md` | `packs/artifact/latex.md`、竞赛 Pack、当前框架 |
| 评分、终审、提交包 | `modules/06_review_delivery.md` | 框架/状态/产物校验器、`scripts/score_submission.py` |
...
- 必含内容：当前数据/模型口径、论文结构、逐问模型与结果摘要、综合检验、图表证据链、待办和同步检查；
...
- `scripts/validate_model_paper_framework.py`：框架结构、逐问结果摘要、锚点和状态同步校验；
- `scripts/validate_project_state.py`：项目状态、框架/结果摘要 freshness 和语义校验；
...
4. 框架、状态、工作簿、图表和论文的职责边界不得混写成冲突事实源；
5. `legacy/` 不参与活动索引、Manifest 和默认执行；
6. GitHub 代码搜索延迟时按本文件路径直接读取；
7. 变更必须通过静态 lint、Python 3.10–3.14 单元测试、Schema、框架校验、生成文件及三套 LaTeX 冒烟编译。
