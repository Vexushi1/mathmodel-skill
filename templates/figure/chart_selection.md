...
局部放大、Small Multiples、Composite Diagnostic、饼图、环形图、雷达图、3D 柱状图、3D 曲面及其他高级表达均允许使用，但必须通过以下检查：

1. **结论匹配**：图型或 Enhancement 直接服务 Core conclusion，而不是为了视觉复杂度；
2. **信息效率**：相较常规图，能更快或更完整地展示比例、层级、多维结构、空间形态、局部差异、对象结构或响应面；
3. **比较可读性**：主要对象、差异、极值和推荐方案能够快速辨认；
4. **视觉诚实性**：透视、面积、角度、遮挡、色彩、截轴、平滑和归一化不会系统性放大或缩小差异；
5. **数据充分性**：数据量、类别数、维度数和网格密度足以支撑该图型；
6. **补充证据**：必要时同时提供二维投影、等高线、数据标签、原始点、ROI、残差或结果表；
7. **可复现性**：能够从指定工作簿、工作表和 `q{x}_plot.m` 重绘。

若高级表达未通过上述检查，改用更直接的二维图型或 `Enhancement=none`。这里的降级依据是信息效率和误导风险，而不是图型名称。
...
7. Semantic Background 使用低视觉权重浅色，不能与主结果争夺注意力。
...
7. 一张 Figure 可以包含多个 axes，但原则上只有一个一级阅读任务；规则矩阵、Small Multiples 和 Composite Diagnostic 的 axes 数量不等于一级任务数量。
