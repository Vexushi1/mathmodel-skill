...
| 项目 | 内容 |
|---|---|
| 程序名称 | 按 `core/output_contract.yaml#per_question.solver_scripts` 填当前主入口（如 `问题X求解/问题X求解.py` 或 `问题X求解/qX_solver.m`）和 `问题X求解/qX_plot.m`；Gate=`required` 时追加当前深化入口（如 `问题X求解/问题X结果深化分析.py` 或 `问题X求解/qX_analysis.m`），并列明已声明的必要辅助源码 |
...
| 复现顺序 | 按主后端运行当前主入口 → 验收主工作簿并冻结主脚本 → Analysis Necessity Gate → `required`：按深化后端运行独立入口并验收 `问题X结果深化分析.xlsx`；`not_required`：记录非空理由并跳过 03B → 运行 `qX_plot.m` → LaTeX 编译 |
