...
% 不复用模板中已有的 theorem/proof 环境，避免中文标题与编号样式冲突。
...
% 命题只在确有理论作用时使用。推荐位置：详细推导之后、核心模型汇总和求解算法之前。
% 全文命题总数不得超过 4；无高价值命题时删除以下示例。
% \begin{proposition}[变换模型的等价性]\label{prop:q1-equivalence}
% 在假设 A1--A3 及参数范围……内，原模型与变换模型具有相同的可行解映射和最优目标值。
% \end{proposition}
% \begin{hskproof}
% 由……可得……，进而证明两个可行域之间存在双射，且目标函数值保持不变。
% \end{hskproof}
% 命题~\ref{prop:q1-equivalence} 表明……，因此后续可在不改变原问题最优解的前提下采用变换模型求解；当……不成立时，该结论失效。
...
% 按实际小问继续添加“模型建立—必要命题与证明—求解方法—结果与分析”。
...
\section{补充推导与图表}
% 长证明可将技术细节放在此处，但正文仍须保留命题条件、关键证明链、建模作用和失效边界。
% 仅放正文不宜展开但仍有证据价值的图表。
\end{appendices}
