...
A1 记录仍是结构对应声明；显式 A2 策略及其交付/验收绑定按 `core/model_code_conformance_contract.yaml#activation.integration` 管理。不要在此表复制状态字段或手填 accepted。旧绑定失效时保留来源，先重新交付和核验原回执，不修改数学批准来掩盖工程变化。
