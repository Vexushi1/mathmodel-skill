...
WORKFLOW = ROOT / ".github" / "workflows" / "matlab-publication-preview.yml"
...
class P6bMatlabPreviewContractTests(unittest.TestCase):
    ...
    def test_workflow_uses_real_mathworks_actions(self) -> None:
        raw = WORKFLOW.read_text(encoding="utf-8")
        data = yaml.safe_load(raw)
        jobs = data["jobs"]
        self.assertEqual(list(jobs), ["real_matlab_preview"])
