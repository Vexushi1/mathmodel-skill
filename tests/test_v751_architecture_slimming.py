...
class ArchitectureSlimmingV751Tests(unittest.TestCase):
    def test_bootstrap_is_pointer_only_and_startup_budget_is_smaller(self):
        ...
        self.assertLessEqual(
            bootstrap_path.stat().st_size + policy_path.stat().st_size,
            22000,
        )
    ...
    def test_reasoning_contract_keeps_v750_capabilities(self):
        ...
        self.assertEqual(
            contract["formula_reasoning_chain"]["chain"],
            ["source", "derivation", "destination"],
        )
        ...
        self.assertIn(
            "final_solver_selection",
            contract["structure_before_algorithm"]["check_order"],
        )
        self.assertIn(
            "optimization_tolerance",
            contract["numerical_parameter_evidence"]["applies_to"],
        )
        self.assertIn(
            "structural_consistency",
            contract["multi_method_validation"]["two_levels"],
        )
        self.assertEqual(
            contract["prose_style"]["name"],
            "evidence_driven_undergraduate_academic",
        )
    ...
    def test_taxonomy_is_lazy_for_nonclassification_routes(self):
        ...
        taxonomy = yaml.safe_load(
            resolver.TAXONOMY_PATH.read_text(encoding="utf-8")
        )
    ...
    def test_model_design_reasoning_sections_are_not_duplicated(self):
        model_design = (ROOT / "modules/02_model_design.md").read_text(
            encoding="utf-8"
        )
        ...
        self.assertLess(
            model_design.index("### 4.3 数值参数证据计划"),
    ...
    def test_one_shot_maintenance_files_do_not_leak_into_active_tree_or_manifest(self):
        one_shot_paths = (
            ...
            "scripts/_v751_restore_anchors.py",
            ".github/workflows/v751-compat-anchors.yml",
        )
    ...
    def test_minimal_router_default_load_remains_single_policy(self):
        ...
        self.assertEqual(
            router["load_policy"]["principle"],
            "minimal_route_specific",
        )
