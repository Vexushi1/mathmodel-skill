...
class V1000ReleaseVersionMatrixTests(unittest.TestCase):
    def test_current_skill_carriers_and_single_backend_cli(self):
        ...
        self.assertEqual(str(bootstrap["skill_version"]), "10.2.0")
        plugin = load(".codex-plugin/plugin.json")
        self.assertEqual(plugin["version"], "10.2.0")
        ...
        for relative in (
            ...
            with self.subTest(relative=relative):
                self.assertEqual(str(load(relative)["version"]), "10.2.0")
        self.assertEqual(
        ...
        self.assertIn("version: 10.2.0", root_skill)
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
        self.assertTrue(readme.startswith("# mathmodel-skill v10.2.0"))
        self.assertEqual(readme.splitlines()[0], "# mathmodel-skill v10.2.0")
        self.assertTrue(changelog.startswith("# Changelog\n\n## 10.2.0\n"))
        self.assertTrue((ROOT / "core/hsk_core_policy.md").read_text(encoding="utf-8").startswith("# HSK Core Policy v10.2.0"))
    ...
    def test_independent_protocol_versions_are_not_skill_versions(self):
        ...
        self.assertEqual(load("core/project_state.schema.yaml")["version"], "8.1.0")
        self.assertEqual(execution["version"], "3.1.0")
        self.assertEqual(load("core/runtime_assurance_contract.yaml")["version"], "2.1.0")
        self.assertEqual(load("core/state_transition_contract.yaml")["version"], "1.3.0")
