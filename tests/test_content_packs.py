...
class TestContentPacks(unittest.TestCase):
    ...
    def test_chart_selection_is_evidence_driven_and_caption_owned(self):
        ...
        self.assertIn("核心证据覆盖检查", text)
        self.assertIn("基础图可以承担核心证据", text)
        self.assertIn("已有证据充分时保留", text)
        self.assertIn("没有实际区间不画带", text)
        self.assertNotIn("基础图退化检查", text)
    ...
    def test_figure_pack_uses_adaptive_analysis_and_standard_script_name(self):
        ...
        self.assertIn("可直接承担核心论证", pack)
        self.assertIn("真实互补信息和可读性增益同时存在", pack)
        self.assertIn("不按 plain 图数量或占比判断质量", pack)
        self.assertIn("不凑第二种", pack)
        self.assertIn("不强制 hero panel", pack)
        self.assertIn("不设默认配色或默认 palette profile", module)
        self.assertIn("没有默认色板、蓝红优先或强制中高饱和要求", module)
        self.assertIn("categorical", module)
        self.assertIn("sequential", module)
        self.assertIn("diverging", module)
