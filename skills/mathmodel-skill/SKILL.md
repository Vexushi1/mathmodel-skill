...
description: Plugin discovery shim for the HSK v6.4.0 lightweight-bootstrap, multi-intent routing, LaTeX-first authoring and contract-closed project synchronization workflow.
---
...
After `locked_model_spec` exists, maintain project-root `模型论文框架.md` as the current effective model and paper source. The default complete workflow proceeds from validated figures directly to LaTeX authoring, AI cleanup, compilation and review. Load the DOCX route only when the user explicitly requests Word review, comments or a DOCX-specific submission; DOCX is not a LaTeX prerequisite.

Before every formal model, code, workbook, MATLAB figure, DOCX, LaTeX or submission delivery, execute the resolver's `pre_delivery_gates`. Run `../../scripts/sync_project.py <project_root> --write --strict --delivery-scope <scope>` for `project_sync`; only a successful gate makes `sync_report.yaml` available.
