# mathmodel-skill v10.0.0（Draft 候选，尚未发布）
...
独立复制的 Python 支持包按同一合同检查运行配置和条件工作表。两份 skill 入口补齐 `description`，原版本/摘要/触发词保留在 `metadata`；仓库读取器兼容旧顶层字段。显式分类按字段报告与当前项目的冲突，未指定轴继续读取当前值，已登记赛事别名与集合顺序不再造成假冲突。入口通过静态格式和资源路径检查，未声称完成真实宿主安装验证。

## v9.6.0：绑定实际完整编译输入

正式 LaTeX 报告升级为 v4，记录实际编译读取的项目文件和 `.fls` 哈希。配置变化、静态审计漏掉的实际输入或 `includeonly` 漏章都会阻止复用旧证明；历史 v3 报告需重新审计、完整编译后取得新正式资格。正文引用附录不再被误判缺失，不同小问及明确否定的主张也分别核对。具体迁移见[全仓库计划](docs/repository_audit_and_handoff_refactor_plan.md)。

## v9.5.7：按当前项目核对提交文件

复现包逐问检查基础文件及按需 03B、项目级预处理、声明数据和图文证据；同扩展名文件不能替代缺失的小问。official 模式的精确必需路径缺失会在生成和验收时同时拒绝，合法 PDF-only 包保持独立。历史包仍可读取；缺少当前状态或材料时需补齐后重打、重验。

## v9.5.6：同步先传播失效，再检查交付

同步现在用刚发现的文件变化形成候选状态，再判定正式交付。项目根目录图件可通过框架中的精确脚本/导出路径或本问 Figure ID 绑定纳入哈希；同步与精简读取共用这一发现逻辑。已有批准不会被重新发现文件自动补写，独立全局图未映射时明确提示覆盖边界。

## v9.5.5：主求解、深化分析与当前证据资格

03B 代码交付和回执必须建立在当前 accepted 主结果及明确的分析必要性判定上；合法 `not_required` 可继续进入验证、写作和完成状态。主结果改变会重新打开判定，成功验收只关闭本阶段的数值失效标记，保留下游图文待更新状态。运行时的兼容名称不能绕过当前文件、哈希或验收状态。迁移与例外见[全仓库计划](docs/repository_audit_and_handoff_refactor_plan.md)。

## v9.5.4：严格布尔证据验收

用于主判定的布尔证据必须复核为真，并使用现有 numerical contract 指定的关系。失败证据不能通过改成数值比较得到 accepted；保留非主判定历史行与合法数值证据。实施与迁移见[全仓库计划](docs/repository_audit_and_handoff_refactor_plan.md)。
