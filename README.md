...
正文写作唯一权威仍位于 `modules/05_writing/latex.md`。v7.4.3 在 v7.4.0 的证据架构基础上进一步落实国赛阅读结构：

- 问题背景原则上单段；
- 问题分析按问分小节，不含正式公式与最终结果；
- 模型假设和符号说明分章；
- 高频符号控制长下标，场景/模型信息可用简短上标；
- 各问采用“问题X模型建立及求解”，详细推导后必须有“核心模型汇总”；
- 命题证明分步骤，常规命题框不跨页；
- 检验尽量贴近对应小问结果，不机械设置统一“模型检验/敏感性与鲁棒性”章节；
- 模型评价写当前模型真实结构取舍，不用万能优缺点；
- AI cleanup 之后继续做科研初学者式自然重写，让语言规范但不过度成熟和模板化。

结果正文仍优先形成“关键数值/现象 → 比较基准 → 机制解释 → 题目结论 → 必要边界”；真实反常结果、局部不稳定、阈值敏感和失效边界不得为了叙事顺滑被删除。
