...
表格 Cleanup 只改善显示层：把内部 run id、代码变量名或裸符号替换/补充为可读显示名时，必须能回到 current Terminology/Numeric/工作簿事实；不得猜测含义。单位、无量纲、百分比/百分点、绝对/相对误差、指标优劣方向、baseline 和排序口径不得为了“表头更短”改变。过宽表优先拆分真实比较维度或删冗余列，不缩小到难读，也不以删除单位和精度换宽度。
