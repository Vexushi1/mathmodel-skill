...
公式密集段落只执行 Authority / Protocol 的 Formula-Rich Narrative：保留“当前对象/缺口 → 依据与推导 → 作用/下一用途”，关键条件、近似范围和失效边界不得因压缩丢失；已定义变量可删重复翻译。信息过载的长句按真实推理单元拆分，同一依赖链可合并；不得按字符数/句数硬切，也不把彼此无直接依赖的模型选择、solver、指标和结果硬塞在一句。
