...
公式密集段落清理时，优先保留“当前对象/已知关系 → 当前缺口 → 依据/推导 → 得到的结构作用 → 下一用途”。已经定义过的变量可删重复翻译，但关键成立条件、近似范围、定义域、局部性和失效边界不能因“前文公式已经很多”被一并删掉；这些条件若已在邻近位置完整出现，可以用明确回指替代重复。

Cleanup 可以把信息过载的长句拆成多个推理单元，也可以把同一依赖链的短句合并，但判断依据是对象、数学依赖、证据角色和下游任务，不是字符数或句子长度。模型选择、solver 介绍、指标定义和结果解释若彼此独立，不应为了“段落更少”硬塞进同一句；如果它们构成同一连续推理链，也不为形式整齐强拆。
