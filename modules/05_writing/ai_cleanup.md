...
公式密集段落只执行 Authority / Protocol 的 Formula-Rich Narrative：保留“对象/缺口 → 依据/推导 → 作用/下一用途”，关键条件与失效边界不得丢失。信息过载的长句按真实推理单元拆分，同一依赖链可合并；不按字符/句数硬切，也不把无直接依赖的模型选择、solver、指标和结果硬塞一句。
