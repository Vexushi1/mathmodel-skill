...
正式标题层级同时服从 Authority 的最大三级规则：一级、二级、三级均正常可用，Cleanup 不得因为三级标题较多就自动压回二级。若活动正式论文出现真实四级及以上章节，应把其语义并入最合适的三级标题或改为普通正文/证明分情况/算法步骤；**不能仅去编号后用加粗独立行、列表或小字号继续保留第四层语义**。Cleanup 只处理论文成稿，不全仓搜索 `####`、`paragraph` 或维护文档层级；自定义宏/样式层级不明时保留并交 Review 复核。
