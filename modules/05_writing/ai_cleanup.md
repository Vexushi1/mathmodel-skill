...
跨竞赛推理、规则等级、模型/求解器/验证器角色、优化模型表达、命题预算、引用证据、术语、数字、Title Claim、深化证据处置、Paragraph Necessity 与局部 stale 治理：`core/writing_reasoning_contract.yaml`。
...
优化模型的表达顺序、模型命名以及 Model/Solver/Validator 的判定只服从 `writing_reasoning_contract.optimization_model_expression`、`model_naming` 与 `model_solver_validator_roles`。AI Cleanup 只发现表现风险，不根据算法名猜测真实模型类型。
...
问题章节内部执行 `writing_reasoning_contract.subsection_granularity`。这里不限制一级章节数量，也不按固定标题数自动否决。

重点复查：

- 一个二级小节只有一个公式、一张表或一幅图；
- “决策变量、目标函数、约束、核心模型汇总”分别机械拆成多个二级小节；
- “活跃边界、删除消融、独立算法挑战、局部邻域”都服务同一结果可信性，却各自单开标题；
- 一个问题内部标题数量明显大于真实独立论证单元，正文被切成技术报告式碎片。

能合并时优先形成“模型建立 → 模型求解 → 结果分析 → 必要检验”的连续结构。减少标题数量不删除技术内容；机器只给 review/warning，不能仅凭“小节 > 4”自动合并或判错。
...
机器审计不得自动重写正文，也不得从正则推断数学正确性、公式真实来源、参数最优性、术语语义等价、模型标准类型、物理/统计准确性、文献质量、引用是否真正语义支持某个 claim，或仅凭小节数量判断结构质量。
