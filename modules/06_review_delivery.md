...
4. Algorithm：`stepwise/pseudocode` 自动激活 current Algorithm Trace 与 algorithm-flow pack，即使用户没有再次说“伪代码”；`not_needed` 不造装饰算法框；current trace 还必须能闭合到**真实代码实现**及对应结果/验证证据；
