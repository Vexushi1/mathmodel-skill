...
本规则只检查**问题章节内部二级/三级小节**的恢复性与颗粒度，不改变 Template Manifest 的一级结构。独立公式组、证明/判据、结构化简、参数证据或 solver stage 可以拆分；薄内容和同一连续链可合并。**这不是硬计数**，复杂问题小节较多不等于错误；三级标题是正常可用层级，不因数量较多或“非必要”字样自动失败。

另按 `formal_heading_depth_policy` 核全文正式章节（含正式附录）：一级至三级可用，明确可解析的四级及以上正式标题属于确定性结构违规并阻止正式交付。LaTeX 中活动正文的 `paragraph/subparagraph`（含星号形式）若承担章节标题即违规；DOCX Heading 4+ 同理。自定义宏或样式的真实大纲层级无法确定时进入 `review_required`，不得从宏名、字体大小或字符串外观猜测。证明分情况、算法步骤、公式编号、表内分组、封面元信息，以及仓库维护 MD/代码注释/未激活示例不算第四层章节。
