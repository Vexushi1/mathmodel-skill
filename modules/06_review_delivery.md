...
小节恢复性与颗粒度规则只检查**问题章节内部二级/三级小节**，不改变 Template Manifest 的一级结构。独立公式组、证明/判据、结构化简、参数证据或 solver stage 可以拆分；薄内容和同一连续链可合并。**这不是硬计数**，复杂问题小节较多不等于错误；三级标题是正常可用层级，不因数量较多或“非必要”字样自动失败。
