...
语义闭环之外，每个核心公式或核心关系再记录 **Source–Derivation–Destination**：

| 核心关系 | Source：题意/定义/机制/理论依据 | Derivation：关键推理 | Destination：后续用途 | 状态 |
|---|---|---|---|---|
|  |  |  |  | closed / gap |

- Source 可来自题面条件、定义、物理/几何机制、概率统计原理、数据生成机制、决策逻辑、状态转移、前文公式/命题、已核验标准定理或共享基础模型；
- Derivation 只保留改变建模含义的关键步骤，纯代数展开可压缩；
- Destination 必须落到状态/参数、下一公式、目标、约束、判定、降维、算法评价、数值参数、验证证据或最终答案之一；
- 既没有题意/机制来源，也没有后续用途的核心公式不能进入 locked model。

这张表是内部模型/写作记忆，不要求原样进入论文；论文正文用自然语言和公式恢复同一推理链。

### 4.2 共享基础与跨问模型增量

先判断多个小问是否共享核心状态方程、几何/概率/网络结构、判定函数或统一评价指标。只有共享推导较长、重复定义会显著降低可读性时，才计划独立“模型准备/共享基础模型”等章节；名称按题目对象生成，不固定。

共享基础只记录跨问共同有效内容，不提前塞入单问最优结果、算法和后问专属约束。对存在实际依赖的后问，记录：

| 小问 | 继承结构 | 新增对象/条件 | 新增数学结构 | 困难变化 | 求解变化 |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

若小问独立，明确标记 independent，不为文章“连贯”虚构依赖。

### 4.3 数值参数证据计划

凡步长、网格、离散点数、样本量、Bootstrap 次数、截断阶数、窗口/滞后、搜索分辨率、优化容差或最大迭代数会改变结论，必须在求解前计划选择证据：

| 参数 | 数学作用 | 候选范围 | 证据方法 | 通过标准 | 最终值状态 |
|---|---|---|---|---|---|
|  |  |  |  |  | pending |

证据按题型选择：数值离散用网格/步长收敛；随机模拟用标准误、置信区间或重复稳定性；统计/时间序列用信息准则、交叉验证、残差或稳定性；机器学习用验证曲线/交叉验证/学习曲线；优化用目标值、可行性、多初值和终止证据。题面固定参数注明来源即可，不重复调参。
